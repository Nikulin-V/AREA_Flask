...
def processing():
    ...
    if data['type'] == 'message_new':
        user: VkUser
        user = db_sess.query(VkUser).filter(VkUser.vk_id == data['object']['message']['from_id']).first()
        last_message = db_sess.query(VkUser.last_message_id).filter(VkUser.vk_id ==
                                                                    data['object']['message']['from_id']).first()
        if user:
            ...
            if data['object']['message']['conversation_message_id'] > int(last_message[0]):
                ...
                if db_sess.query(VkUser.AREA_email).filter(VkUser.vk_id ==
                                                           data['object']['message']['from_id']).first():

                    """добавление id последнего сообщения (для устранения повторных запросов)"""
                    vk_user = db_sess.query(VkUser).filter(VkUser.vk_id == data['object']['message']['from_id']).first()
